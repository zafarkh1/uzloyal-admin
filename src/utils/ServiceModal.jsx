...
export function ServiceModal({ getApi, data }) {
  ...
  const { serviceId, setServiceId } = useIdStore();

  const item = data.find((el) => el.id === serviceId);
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    if (formValidation()) {
      ...
      formData.append("images", img);
      try {
        await apiRequest(`services/${serviceId}`, "Put", formData);
        handleCloseModal();
  ...
  return (
    <div>
      <Modal
        titleUz=""
        open={serviceId ? isEditCarOpen : isCreateCarOpen}
        onOk={serviceId ? handleUpdateSubmit : handleCreateSubmit}
        onCancel={handleCloseModal}
      >
        <h5 className="text-xl font-semibold text-center">
          {serviceId ? "Edit item" : "Adding item"}
        </h5>
