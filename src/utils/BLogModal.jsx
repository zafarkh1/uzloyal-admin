...
export function BlogModal({ getApi, data }) {
  ...
  const { blogsId, setBlogsId } = useIdStore();

  const item = data.find((el) => el.id === blogsId);
  ...
  const handleCreateSubmit = async (e) => {
    ...
    if (formValidation()) {
      ...
      try {
        await apiRequest("blogs", "Post", formData);
        handleCloseModal();
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    if (formValidation()) {
      ...
      formData.append("images", img);
      try {
        await apiRequest(`blogs/${blogsId}`, "Put", formData);
        handleCloseModal();
        message.success("Blog updated successfully!");
        getApi();
      } catch (error) {
        message.error("Failed to updated blog");
      }
    } else {
      message.error("Please fix validation errors");
    }
  ...
  return (
    <div>
      <Modal
        titleUz=""
        open={blogsId ? isEditModelOpen : isCreateModelOpen}
        onOk={blogsId ? handleUpdateSubmit : handleCreateSubmit}
        onCancel={handleCloseModal}
      >
        <h5 className="text-xl font-semibold text-center">
          {blogsId ? "Edit item" : "Adding item"}
        </h5>
